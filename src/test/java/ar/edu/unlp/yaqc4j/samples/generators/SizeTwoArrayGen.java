...
public class SizeTwoArrayGen implements Gen<int[]> {
     ...
     * @{inheritDoc
     */
    public final int[] arbitrary(final Distribution random, final long minsize,
	    final long maxsize) {
	int[] res = new int[2];
	res[0] = (int) Arbitrary.choose(random, minsize, maxsize);
	res[1] = (int) Arbitrary.choose(random, minsize, maxsize);
	return res;
    }
}
